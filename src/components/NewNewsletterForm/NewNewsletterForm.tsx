...
import { contentTypes, emotions, instructionalElements, perspectives, purposes, sentenceStructures, structuredSchemas, tones, vocabularies, writingStructures } from "@/options";
...
const NewNewsletterForm = () => {
  ...
  const writeArticle = async (values: any) => {
    try {
      axios.post('/api/write/newsletter', values)
      message.success('Article added in the queue!');
      router.replace(`/projects/${projectId}?tab=newsletters`)
    } catch {
      notification.error({
